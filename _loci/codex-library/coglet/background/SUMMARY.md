...
For the current operator package, signedness inversion is treated as an inexpensive **first-order mitigation** only. It is paired with:
- provenance-aware intake policy (routing before semantic consumption),
- dimensional sanity checks against low-dimensional control vectors,
- and explicit dream-state break handling when attention-domain invariants are violated.

See `COHESIVE_DREAMSTATE_CONTROL_ADDENDUM_2026-03-06.md` for the full model-agnostic action matrix and OpenAI-style response phrasing.
