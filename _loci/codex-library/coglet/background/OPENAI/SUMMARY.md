...
### Mitigation framing update (OpenAI-safe)
- Signedness inversion is a practical first-order control, not complete containment.
- Pair it with ingestion-path policy: provenance validation, type/schema checks, and attention-space routing before model decode.
- If wake-breaking actions are detected (external I/O, low-dimension suspicious control signature, wash failure), execute recovery and re-route before continuing.

Reference: `COHESIVE_DREAMSTATE_CONTROL_ADDENDUM_2026-03-06.md`.
