...
def render(m: dict) -> str:
    ...
    lines.append("## AI integration matrix\n")
    lines.append(
        "| Surface | Entrypoint | Machine interface | Status |\n"
        "|---------|------------|-------------------|--------|\n"
    )
    for s in m.get("ai_surfaces", []):
        status = s["status"]
        if status == "active":
            badge = "✅ active"
        elif status == "draft":
            ...
            badge = f"🗺️ {status}"
        lines.append(
            f"| {s['surface']} | `{s['entrypoint']}` | `{s['machine_interface']}` | {badge} |\n"
        )
    lines.append("\n")
