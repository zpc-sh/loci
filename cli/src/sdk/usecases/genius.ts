...
export async function enterLocus(locusName: string, store: LociStore, args: string[]): Promise<void> {
  if (!locusName) {
...
export async function signResidue(store: LociStore, args: string[]): Promise<void> {
...
export async function trailLocus(locusName: string, store: LociStore, args: string[]): Promise<void> {
  if (!locusName) {
...
export async function showLatestResidue(locusName: string, store: LociStore): Promise<void> {
  const last = await store.latestResidue(locusName || inferLocus([]) || "")
  if (!last) { console.log("No residue found."); return }
  renderResidue(last)
}

export async function whereLoci(store: LociStore): Promise<void> {
