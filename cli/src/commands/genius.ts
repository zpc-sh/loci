...
export async function cmdSign(store: LociStore, args: string[]): Promise<void> {
  ...
  if (process.stdin.isTTY) {
    ...
    if (clack.isCancel(did)) { clack.cancel('Operation cancelled.'); process.exit(0) }
    if (did) residue = { ...residue, whatIDid: [did as string] }

    const left = await clack.text({ message: "What's left open?", placeholder: "(none)" })
    if (clack.isCancel(left)) { clack.cancel('Operation cancelled.'); process.exit(0) }
    if (left && left !== "(none)") {
    ...
    if (clack.isCancel(rec)) { clack.cancel('Operation cancelled.'); process.exit(0) }
    if (rec && rec !== "(skip)") {
