...
Use the included repo-scan wrapper to run loci_fsm surfaces against this
live workspace and emit reproducible artifacts:

```bash
just loci-fsm-repo-scan
```

Outputs are written to `artifacts/loci-fsm/`:

- `wasm-plan.txt` — `daemon yata wasm-plan` output (includes boundary stigmergy)
- `triad-contract.txt` — `daemon yata triad-contract` output
- `summary.env` — pinned scan inputs (heads, branches, routes, tokens, peers)

Optional environment overrides:

- `ROUTES` — comma-separated route list
- `TOKENS` — comma-separated token list
- `MU_HEAD`, `LANG_HEAD`, `MU_BRANCH`, `LANG_BRANCH`
- `DRIFT_PEERS`

For a full assurance + wasm examples flow, see:
